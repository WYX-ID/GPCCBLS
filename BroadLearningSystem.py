...
from funcs import evaluate_metrics, tsne_plot, umap_plot
# 评估分类模型


def evaluate(label, label_pred):
    y_test = np.zeros(label.shape[0])
    y_pred = []
    y_test = label.argmax(axis=1)
    y_pred = label_pred.argmax(axis=1)
    ...
    '''
    plt.figure()
    TPR, FPR, threshhold = roc_curve(y_test, y_pred)
    plt.plot(TPR, FPR)
    plt.ylabel(0, 1)
    plt.title('ROC曲线')
    plt.show()
...
def sparse_bls(A, b):  # 稀疏编码器（Sparse AutoEncoder）——SRC
    lam = 0.001
    itrs = 50
    AA = A.T.dot(A)
    m = A.shape[1]
...
def CR_sparse_bls(Z, X):  # CRC
    np.random.seed(123)
    reg_range = np.logspace(-7, 3, 10)
    reg = np.random.choice(reg_range)
    P = np.mat(reg*np.eye(Z.shape[1])+Z.T.dot(Z)).I.dot(Z.T)
    return np.dot(P, X)


def CCR_sparse_bls(Z, X, lambda1=None, lambda2=None):  # CCRC——效果比sparse_bls好一点

    if lambda1 is None:
        np.random.seed(20220406)
        lamda_range = np.logspace(-7, 3, 10)
        lamda1 = np.random.choice(lamda_range)
        lamda2 = np.random.choice(lamda_range)
    else:
        lamda1, lamda2 = lambda1, lambda2
    k = Z.shape[1]
    M = np.mat(np.identity(k))
    for i in range(k):
        M[i, i] = Z[:, i].T.dot(Z[:, i])  # Z[:, i].T*Z[:, i]
    A = (1+lamda2)*(Z.T.dot(Z)+lamda1*np.mat(np.identity(k))+lamda2*M)
    P = np.mat(A).I.dot(Z.T)  # pinv(A, c).dot(Z.T)或许可以使用伪逆
    W = np.dot(P, X)
    return W


def CCR_enhancement(Z, X):
    k = Z.shape[1]
    M = np.mat(np.identity(k))
    for i in range(k):
        M[i, i] = Z[:, i].T.dot(Z[:, i])  # Z[:, i].T*Z[:, i]
    P = np.mat(M).I.dot(Z.T)
    W = np.dot(P, X)
    return W


def SCCR_sparse_bls(Z, X):  # SCCRC——效果不是很好
    alpha = sparse_bls(Z, X)
    beta = CCR_sparse_bls(Z, X)
...
def CCR_BLS(train_x, train_y, test_x, test_y, s, c, N1, N2, N3, sparse='sparse_bls'):

    # 可视化输入数据
    # tsne_plot(train_x, train_y)
    umap_plot(train_x, train_y)
    L = 0
    train_x = preprocessing.scale(train_x, axis=1)
    FeatureOfInputDataWithBias = np.hstack(
        [train_x, 0.1 * np.ones((train_x.shape[0], 1))])
    ...
    for i in range(N2):
        random.seed(i)
        weightOfEachWindow = 2 * \
            random.randn(train_x.shape[1]+1, N1)-1  # 随机生成权重
        FeatureOfEachWindow = np.dot(
            FeatureOfInputDataWithBias, weightOfEachWindow)

        scaler1 = preprocessing.MinMaxScaler(
            feature_range=(0, 1)).fit(FeatureOfEachWindow)
        FeatureOfEachWindowAfterPreprocess = scaler1.transform(
            FeatureOfEachWindow)

        betaOfEachWindow = eval(
            sparse+'(FeatureOfEachWindowAfterPreprocess, FeatureOfInputDataWithBias).T')  # 使用不同方式调整权重

        Beta1OfEachWindow.append(betaOfEachWindow)
        outputOfEachWindow = np.dot(
            FeatureOfInputDataWithBias, betaOfEachWindow)
#        print('Feature nodes in window: max:',np.max(outputOfEachWindow),'min:',np.min(outputOfEachWindow))
        distOfMaxAndMin.append(
            np.max(outputOfEachWindow, axis=0) - np.min(outputOfEachWindow, axis=0))
        minOfEachWindow.append(np.min(outputOfEachWindow, axis=0))
        outputOfEachWindow = (outputOfEachWindow -
                              minOfEachWindow[i])/distOfMaxAndMin[i]
        OutputOfFeatureMappingLayer[:, N1*i:N1*(i+1)] = outputOfEachWindow
        del outputOfEachWindow
        del FeatureOfEachWindow
        del weightOfEachWindow
    # plot features with T-SNE
    # tsne_plot(OutputOfFeatureMappingLayer, train_y)
    umap_plot(OutputOfFeatureMappingLayer, train_y)
    ...
    tempOfOutputOfEnhanceLayer = np.dot(
        InputOfEnhanceLayerWithBias, weightOfEnhanceLayer)

    # CCR enhancement
    if sparse == 'CCR_sparse_bls':
        weightOfEnhanceLayer = CCR_enhancement(
            tempOfOutputOfEnhanceLayer, InputOfEnhanceLayerWithBias).T
        tempOfOutputOfEnhanceLayer = np.dot(
            InputOfEnhanceLayerWithBias, weightOfEnhanceLayer)
#    print('Enhance nodes: max:',np.max(tempOfOutputOfEnhanceLayer),'min:',np.min(tempOfOutputOfEnhanceLayer))
    ...
    # 增强层映射函数
    OutputOfEnhanceLayer = tansig(
        tempOfOutputOfEnhanceLayer * parameterOfShrink)

    # 生成最终输入
    InputOfOutputLayer = np.hstack(
        [OutputOfFeatureMappingLayer, OutputOfEnhanceLayer])
    # 可视化最终输入
    # tsne_plot(InputOfOutputLayer, train_y)
    # umap_plot(InputOfOutputLayer, train_y)
    ...
    evaluate(train_y, OutputOfTrain)
    ...
    evaluate(test_y, OutputOfTest)  # 评估模型
    metrics = evaluate_metrics(test_y, OutputOfTest)
    test_acc[0][0] = testAcc
    test_time[0][0] = testTime

    # test_acc, test_time, train_acc_all, train_time
    return metrics  # testAcc, testTime, trainAcc, trainTime
...
def BLS_AddEnhanceNodes(train_x, train_y, test_x, test_y, s, c, N1, N2, N3, L, M):
    ...
    for i in range(N2):
        ...
        FeatureOfEachWindowAfterPreprocess = scaler1.transform(
            FeatureOfEachWindow)
        betaOfEachWindow = sparse_bls(
            FeatureOfEachWindowAfterPreprocess, FeatureOfInputDataWithBias).T
        Beta1OfEachWindow.append(betaOfEachWindow)
        outputOfEachWindow = np.dot(
            FeatureOfInputDataWithBias, betaOfEachWindow)
        distOfMaxAndMin.append(
    ...
    for e in list(range(L)):
        ...
        if N1*N2 >= M:
            ...
            random.seed(e)
            weightOfEnhanceLayerAdd = LA.orth(
                2 * random.randn(N2*N1+1, M).T-1).T

        tempOfOutputOfEnhanceLayerAdd = np.dot(
            InputOfEnhanceLayerWithBias, weightOfEnhanceLayerAdd)
        parameterOfShrinkAdd.append(s/np.max(tempOfOutputOfEnhanceLayerAdd))
        OutputOfEnhanceLayerAdd = tansig(
            tempOfOutputOfEnhanceLayerAdd*parameterOfShrinkAdd[e])
        tempOfLastLayerInput = np.hstack(
            [InputOfOutputLayer, OutputOfEnhanceLayerAdd])

        D = pinvOfInput.dot(OutputOfEnhanceLayerAdd)
        C = OutputOfEnhanceLayerAdd - InputOfOutputLayer.dot(D)
        if C.all() == 0:
            w = D.shape[1]
            B = np.mat(np.eye(w) - np.dot(D.T, D)
                       ).I.dot(np.dot(D.T, pinvOfInput))
        else:
            B = pinv(C, c)
        ...
        print('Incremental Training Accuracy is :',
              TrainingAccuracy * 100, ' %')

        time_start = time.time()
        OutputOfEnhanceLayerAddTest = tansig(InputOfEnhanceLayerWithBiasTest.dot(
            weightOfEnhanceLayerAdd) * parameterOfShrinkAdd[e])
        InputOfOutputLayerTest = np.hstack(
            [InputOfOutputLayerTest, OutputOfEnhanceLayerAddTest])
...
def BLS_AddFeatureEnhanceNodes(train_x, train_y, test_x, test_y, s, c, N1, N2, N3, L, M1, M2, M3):
    ...
    for i in range(N2):
        ...
        FeatureOfEachWindow = np.dot(
            FeatureOfInputDataWithBias, weightOfEachWindow)
        scaler1 = preprocessing.MinMaxScaler(
            feature_range=(-1, 1)).fit(FeatureOfEachWindow)
        FeatureOfEachWindowAfterPreprocess = scaler1.transform(
            FeatureOfEachWindow)
        betaOfEachWindow = sparse_bls(
            FeatureOfEachWindowAfterPreprocess, FeatureOfInputDataWithBias).T
    ...
    # 生成最终输入
    InputOfOutputLayerTrain = np.hstack(
        [OutputOfFeatureMappingLayer, OutputOfEnhanceLayer])
    ...
    for e in list(range(L)):
        ...
        scaler2 = preprocessing.MinMaxScaler(
            feature_range=(-1, 1)).fit(NewMappingOutput)
        FeatureOfEachWindowAfterPreprocess = scaler2.transform(
            NewMappingOutput)
        betaOfNewWindow = sparse_bls(
            FeatureOfEachWindowAfterPreprocess, FeatureOfInputDataWithBias).T
        Beta1OfEachWindow.append(betaOfNewWindow)

        TempOfFeatureOutput = FeatureOfInputDataWithBias.dot(betaOfNewWindow)
        distOfMaxAndMin.append(
            np.max(TempOfFeatureOutput, axis=0) - np.min(TempOfFeatureOutput, axis=0))
        minOfEachWindow.append(np.mean(TempOfFeatureOutput, axis=0))
        outputOfNewWindow = (TempOfFeatureOutput -
                             minOfEachWindow[N2+e])/distOfMaxAndMin[N2+e]

        OutputOfFeatureMappingLayer = np.hstack(
            [OutputOfFeatureMappingLayer, outputOfNewWindow])

        NewInputOfEnhanceLayerWithBias = np.hstack(
            [outputOfNewWindow, 0.1 * np.ones((outputOfNewWindow.shape[0], 1))])

        if M1 >= M2:
            random.seed(67797325)
            RelateEnhanceWeightOfNewFeatureNodes = LA.orth(
                2*random.random([M1+1, M2])-1)
        else:
            random.seed(67797325)
            RelateEnhanceWeightOfNewFeatureNodes = LA.orth(
                2*random.random([M1+1, M2]).T-1).T
        WeightOfNewFeature2.append(RelateEnhanceWeightOfNewFeatureNodes)

        tempOfNewFeatureEhanceNodes = NewInputOfEnhanceLayerWithBias.dot(
            RelateEnhanceWeightOfNewFeatureNodes)

        parameter1 = s/np.max(tempOfNewFeatureEhanceNodes)

        outputOfNewFeatureEhanceNodes = tansig(
            tempOfNewFeatureEhanceNodes * parameter1)

        if N2*N1+e*M1 >= M3:
            random.seed(67797325+e)
            weightOfNewEnhanceNodes = LA.orth(
                2 * random.randn(N2*N1+(e+1)*M1+1, M3) - 1)
        else:
            random.seed(67797325+e)
            weightOfNewEnhanceNodes = LA.orth(
                2 * random.randn(N2*N1+(e+1)*M1+1, M3).T-1).T
        WeightOfNewFeature3.append(weightOfNewEnhanceNodes)

        InputOfEnhanceLayerWithBias = np.hstack(
            [OutputOfFeatureMappingLayer, 0.1 * np.ones((OutputOfFeatureMappingLayer.shape[0], 1))])

        tempOfNewEnhanceNodes = InputOfEnhanceLayerWithBias.dot(
            weightOfNewEnhanceNodes)
        ...
        tempOfInputOfLastLayes = np.hstack(
            [InputOfOutputLayerTrain, OutputOfTotalNewAddNodes])
        ...
        outputOfNewWindowTest = FeatureOfInputDataWithBiasTest.dot(
            WeightOfNewMapping)

        outputOfNewWindowTest = (
            outputOfNewWindowTest-minOfEachWindow[N2+e])/distOfMaxAndMin[N2+e]

        OutputOfFeatureMappingLayerTest = np.hstack(
            [OutputOfFeatureMappingLayerTest, outputOfNewWindowTest])
        ...
        OutputOfRelateEnhanceNodes = tansig(NewInputOfEnhanceLayerWithBiasTest.dot(
            weightOfRelateNewEnhanceNodes) * parameter1)

        weightOfNewEnhanceNodes = WeightOfNewFeature3[e]

        OutputOfNewEnhanceNodes = tansig(
            InputOfEnhanceLayerWithBiasTest.dot(weightOfNewEnhanceNodes)*parameter2)
